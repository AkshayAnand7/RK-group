...
import { 
  IndianRupee, TrendingUp, TrendingDown, Store, Bus, 
  ArrowUpRight, ArrowDownRight, Wallet, Fuel, 
  AlertCircle, LayoutDashboard, Calendar, Filter
} from "lucide-react";
import { 
...
const revenueData = [
  { day: "Mon", lottery: 32000, travel: 18000 },
  { day: "Tue", lottery: 28000, travel: 22000 },
  { day: "Wed", lottery: 35000, travel: 15000 },
  { day: "Thu", lottery: 41000, travel: 25000 },
  { day: "Fri", lottery: 38000, travel: 20000 },
  { day: "Sat", lottery: 45000, travel: 28000 },
  { day: "Sun", lottery: 26000, travel: 12000 },
];
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
          {[
            { label: "Total Collection", value: "₹2,45,000", color: "text-lottery", bg: "bg-lottery-subtle" },
            { label: "Total Expense", value: "₹45,200", color: "text-danger", bg: "bg-danger-subtle" },
            { label: "Total Advance", value: "₹12,000", color: "text-warning", bg: "bg-warning-subtle" },
            { label: "Prize Money", value: "₹30,000", color: "text-info", bg: "bg-info-subtle" },
            { label: "Net Balance", value: "₹1,57,800", color: "text-white", bg: "bg-lottery", highlight: true },
          ].map((card, i) => (
            <div key={card.label} className={`p-5 rounded-3xl border border-border flex flex-col justify-between transition-all hover:shadow-xl ${card.highlight ? card.bg + " shadow-xl shadow-lottery/20 border-transparent" : "bg-white"}`}>
              ...
              <p className={`text-xl font-black font-mono-nums ${card.highlight ? "text-white" : card.color}`}>{card.value}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: "Total Trip Income", value: "₹1,85,000", color: "text-travel", icon: IndianRupee },
            { label: "Vehicle Expenses", value: "₹62,400", color: "text-danger", icon: Fuel },
            { label: "Pending Amounts", value: "₹18,500", color: "text-warning", icon: AlertCircle },
            { label: "Net Profit/Loss", value: "₹1,04,100", color: "text-white", bg: "bg-travel", highlight: true },
          ].map((card, i) => (
            <div key={card.label} className={`p-6 rounded-3xl border border-border flex flex-col transition-all hover:shadow-xl ${card.highlight ? card.bg + " shadow-xl shadow-travel/20 border-transparent" : "bg-white"}`}>
              ...
              <p className={`text-2xl font-black font-mono-nums ${card.highlight ? "text-white" : card.color}`}>{card.value}</p>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="glass p-6 rounded-4xl flex flex-col justify-center">
          ...
          <div className="flex items-center justify-center gap-12">
            <div className="text-center">
              ...
              <p className="text-2xl font-black text-text-primary font-mono-nums">₹4.3L</p>
            ...
            <div className="text-center">
              ...
              <p className="text-2xl font-black text-text-primary font-mono-nums">₹1.1L</p>
          ...
          <div className="mt-10 p-6 rounded-3xl bg-primary text-white shadow-2xl shadow-primary/30 text-center">
            ...
            <p className="text-4xl font-black font-mono-nums tracking-tight">₹3,20,500</p>
            <p className="text-[10px] font-bold mt-2 text-primary-subtle">+14.2% from last month</p>
          </div>
        </div>
      </div>
    </div>
