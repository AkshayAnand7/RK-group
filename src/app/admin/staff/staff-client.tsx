...
import { Plus, Search, Edit3, Trash2, X, Loader2, UserCheck, Hash, Phone } from "lucide-react";
...
const deptColors: Record<string, string> = {
  ...
  lottery: "text-emerald-600 bg-emerald-50 border-emerald-200",
...
export default function StaffClient({ initialStaff }: { initialStaff: any[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = editing ? await updateStaff(editing.id, fd) : await addStaff(fd);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 scroll-reveal">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-text-primary uppercase tracking-tight">Staff Directory</h1>
          <p className="text-xs sm:text-sm text-text-secondary font-medium">Create and manage staff members</p>
        </div>
        <button onClick={() => { setEditing(null); setShowForm(true); }} className="btn-primary"><Plus className="w-5 h-5" /> Add Staff</button>
      </div>
      ...
      <div className="glass p-3 sm:p-4 rounded-2xl sm:rounded-3xl flex items-center gap-4 scroll-reveal">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or ID..." className="flex-1 bg-transparent border-none outline-none font-bold text-text-primary placeholder:text-text-muted" />
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 scroll-reveal">
        {[
          { label: "Total", value: list.length, c: "text-primary" },
          { label: "Active", value: list.filter(s => s.status === 'active').length, c: "text-emerald-600" },
          { label: "Travel", value: list.filter(s => s.department === 'travel').length, c: "text-blue-600" },
          { label: "Lottery", value: list.filter(s => s.department === 'lottery').length, c: "text-emerald-600" },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 scroll-stagger">
        {filtered.map(s => (
          <div key={s.id} className="glass p-5 sm:p-8 rounded-3xl sm:rounded-4xl border border-border group hover:bg-white hover:shadow-2xl transition-all duration-500 relative overflow-hidden">
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="w-14 h-14 bg-primary rounded-2xl flex items-center justify-center shadow-xl shadow-primary/30 group-hover:rotate-6 transition-transform">
                <span className="text-xl font-black text-white">{s.full_name[0]}</span>
              </div>
              <div className="flex flex-col items-end gap-1.5">
                <span className={`px-3 py-1 text-[10px] font-black uppercase rounded-full border ${statColors[s.status] || statColors.active}`}>{s.status || 'active'}</span>
                <span className={`px-3 py-1 text-[10px] font-black uppercase rounded-full border ${deptColors[s.department] || deptColors.general}`}>{s.department || 'general'}</span>
              </div>
            </div>
            <div className="mb-6">
            ...
            <div className="flex items-center gap-2 pt-6 border-t border-border/50">
              ...
              <button onClick={() => handleDelete(s.id)} className="w-11 h-11 bg-page hover:bg-danger-subtle text-text-muted hover:text-red-500 rounded-xl flex items-center justify-center transition-all cursor-pointer"><Trash2 className="w-4 h-4" /></button>
            </div>
          </div>
        ))}
      </div>
      ...
      {filtered.length === 0 && (
        <div className="glass p-12 rounded-3xl text-center">
          ...
          <p className="text-lg font-black text-text-primary uppercase">No Staff Found</p>
          <p className="text-sm text-text-secondary font-medium mt-1">Add your first staff member</p>
        </div>
      )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              {!editing && (
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] ml-1">Staff ID</label>
                  <input name="staff_id" required placeholder="e.g. STAFF-01" className="w-full h-12 px-4 bg-page border border-border rounded-2xl text-sm font-bold focus:outline-none focus:border-primary transition-all uppercase" />
                </div>
              )}
              <div className="space-y-1.5">
              ...
              {editing && (
                <div className="space-y-1.5">
                  ...
                  <select name="status" defaultValue={editing?.status || 'active'} className="w-full h-12 px-4 bg-page border border-border rounded-2xl text-sm font-bold focus:outline-none focus:border-primary transition-all cursor-pointer">
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button type="submit" disabled={loading} className="flex-1 h-12 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-primary/25 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer flex items-center justify-center">
