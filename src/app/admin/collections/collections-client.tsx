...
export default function CollectionsClient({ initialEntries, initialShops = [] }: { initialEntries: any[], initialShops?: any[] }) {
  ...
  // Export functions for individual shop history
  const handleShopExportPDF = () => {
    if (!selectedShop || selectedShopEntries.length === 0) return;
    const headers = [["Date", "Collection", "Expense", "Advance", "Prize", "Net Balance", "Staff", "Status"]];
    const data = selectedShopEntries.map(e => [
      new Date(e.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }),
      `₹${Number(e.amount).toLocaleString('en-IN')}`,
      `₹${e.expense || 0}`,
      `₹${e.advance || 0}`,
      `₹${e.prize || 0}`,
      `₹${Number(e.amount - (e.expense || 0)).toLocaleString('en-IN')}`,
      e.staff_name || 'Unknown',
      e.is_locked ? 'Locked' : 'Open'
    ]);
    exportToPDF(`${selectedShop} — Collection Report`, headers, data, `${selectedShop.toLowerCase().replace(/\s+/g, '_')}_collections`);
  };

  const handleShopExportExcel = () => {
    if (!selectedShop || selectedShopEntries.length === 0) return;
    const data = selectedShopEntries.map(e => ({
      Date: new Date(e.created_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }),
      Collection: Number(e.amount),
      Expense: e.expense || 0,
      Advance: e.advance || 0,
      Prize: e.prize || 0,
      'Net Balance': Number(e.amount) - Number(e.expense || 0),
      Staff: e.staff_name || 'Unknown',
      Status: e.is_locked ? 'Locked' : 'Open'
    }));
    exportToExcel(data, `${selectedShop.toLowerCase().replace(/\s+/g, '_')}_collections`);
  };
            ...
            <div className="flex items-center justify-between p-4 sm:p-5 lg:p-6 border-b border-border/50 bg-page/50 shrink-0">
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0 ml-3">
                {selectedShopEntries.length > 0 && (
                  <>
                    <button
                      onClick={handleShopExportPDF}
                      className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 bg-primary text-white rounded-xl text-[10px] sm:text-xs font-bold shadow-md shadow-primary/20 hover:bg-primary/90 transition-all cursor-pointer"
                      title="Download PDF"
                    >
                    ...
                    <button
                      onClick={handleShopExportExcel}
                      className="flex items-center gap-1.5 px-2.5 sm:px-3 py-2 bg-emerald-600 text-white rounded-xl text-[10px] sm:text-xs font-bold shadow-md shadow-emerald-600/20 hover:bg-emerald-700 transition-all cursor-pointer"
